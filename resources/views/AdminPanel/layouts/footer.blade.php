...
<!-- jQuery -->
<script type="text/javascript" src="{{ URL::to('/') }}/AdminAssets/dist/js/jquery.min.js"></script>
...
@if(Session::get('PopSuccess') != '')
    <script type="text/javascript">
        $(function () {
            swal("{{ trans('Site.Done') }}", "{{ Session::get('PopSuccess') }}", "success");
        });
...
@if(Session::get('Failed') != '')
    <script type="text/javascript">
        $(function () {
            swal("حدث خطأ", "{{ Session::get('Failed') }}", "error");
        });
        {{ Session::forget('PopSuccess') }}
    </script>
    @endif
